			...
			for {
				...
				case <-ticker.C:
					...
					if err != nil {
						...
						log.Printf("Error when sending refreshed templates. Closing the connection to IPFIX controller")
...
func (ep *ExportingProcess) CloseConnToCollector() {
	...
	if err != nil {
		log.Printf("error when closing connection to collector: %v", err)
	}
}
