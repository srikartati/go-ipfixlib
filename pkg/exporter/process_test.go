...
func TestExportingProcess_AddRecordToMsg(t *testing.T) {
	...
	funcToTestAddRecordMsg(mockExporter, entities.Template, &tempRecBytes)
}
...
func TestExportingProcess_SendingTemplateRecordToLocalServer(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("Got error when creating local server: %v", err)
	}
	t.Log("Created local server on port 3333 for testing")
	...
	if err != nil {
		t.Fatalf("Got error when connecting to local server: %v", err)
	}
	t.Log("Created exporter connecting to local server on port 3333")
